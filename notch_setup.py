...
class lp_setup:
    " Setup low-pass filter in lp1.v "
    def __init__(self,
    ...
    def integers(self):
        " Convert abstract setup to specific lp1.v hardware register values "
        # zn = self.pole / abs(self.pole)
        # num = self.gain * (zn-self.pole)
        # pm1 = self.pole - 1.0
        kx, ky = self.dsp()
...
class notch_setup:
    " Setup low-pass and notch filters in lp1.v and lp_notch.v "
    def __init__(self,
        ...
        if self.notch is None:
            ...
            zbp = exp(-2*pi*(self.bw+1j*self.notch)*self.T)  # pole position
            ...
            zn = exp(-2*pi*1j*self.notch*self.T)  # z at notch frequency
    ...
    def integers(self):
        lp1a = self.cl_lp.integers()
        lp1b = ([0, 0], [0, 0])
        if self.notch is not None:
            lp1b = self.cl_bp.integers()
        return lp1a, lp1b

    def dict(self, base):
        d1 = self.cl_lp.dict(base+"lp1a_")
        d1.update(self.cl_bp.dict(base+"lp1b_"))
        # if self.notch is not None:
        return d1
...
def lp_run(ns):
    regs = ns.integers()
    command = 'vvp -n lp1_tb +out_file=notch_test.dat'
    lp1a = regs[0]
    command += ' +kxr=%d +kxi=%d' % (lp1a[0][0], lp1a[0][1])
    command += ' +kyr=%d +kyi=%d' % (lp1a[1][0], lp1a[1][1])
    print(command)
...
def notch_run(ns, dth=0.0):
    ...
    command += ' +dth=%f' % dth
    lp1a = regs[0]
    command += ' +kaxr=%d +kaxi=%d' % (lp1a[0][0], lp1a[0][1])
    command += ' +kayr=%d +kayi=%d' % (lp1a[1][0], lp1a[1][1])
    lp1b = regs[1]
    command += ' +kbxr=%d +kbxi=%d' % (lp1b[0][0], lp1b[0][1])
    command += ' +kbyr=%d +kbyi=%d' % (lp1b[1][0], lp1b[1][1])
    print(command)
    return_code = call(command, shell=True)
    if return_code != 0:
        print("vvp return_code %d" % return_code)
...
def check_lp(a, bw, dsp, stim_amp=20000, offset=0, plot=False):
    ...
    s_theory = stim_amp * (1-exp(-t*bw*2*pi)) * exp(t*2*pi*1j*offset)
    kx, ky = dsp
    a = [kx, 0]
    b = [1, -1, -ky]
...
if __name__ == "__main__":
    ...
    if True:
        ...
        lp_ok = check_lp(a=a, bw=bw, dsp=ns.cl_lp.dsp(),
                         stim_amp=4*20000, offset=offset1, plot=plot)
    notchf = -800e3
    ...
    regs = ns.integers()
    if True:
        print("values for hardware: lp1a_kx lp1a_ky lp1b_kx lp1b_ky")
        print(regs)
    else:
